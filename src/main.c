...
void AgbMain(void)
{
    ...
    *(vu16 *)BG_PLTT = RGB_WHITE; // Set the backdrop to white on startup
    InitGpuRegManager();
#if PLATFORM_PC
    // GBA hardware boots with DISPCNT=0x0080 (forced blank). Initialize the
    // PC I/O register shadow to match so early SetGpuReg calls that check
    // DISPCNT_FORCED_BLANK write immediately instead of going to the waiting list.
    WRITE_REG_U16(REG_OFFSET_DISPCNT, DISPCNT_FORCED_BLANK);
