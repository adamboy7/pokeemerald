...
This is based off the decompilation of Pokémon Emerald, kindly provided by [Pret and all contributors](https://github.com/pret/pokeemerald). Changes have been made to allow the GBA version to compile, but also enable stubs and emulation-lite functionality (register handling, piping sound and video, clock, flash storage) to enable building for a standard x86 PC. Any PC specific code is gated behing a compiler platform check, so the GBA rom compiles identical to release and the PC port maintains as faithful of a direct port as reasonably possible.
